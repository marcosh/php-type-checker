...
final class MissingMethodParamTypeWithDocBlockTest extends TestCase
{
    public function testCorrectMessage()
    {
        ...
        foreach(MissingMethodParamTypeWithDocBlock::param($parameter) as $anomaly) {
            self::assertSame(
                'Parameter <info>x</info> of method <info>foo</info> of the class <info>Foo</info> ' .
                'defined in <comment></comment> does not have a type hint but has a boc block type of ' .
                '<info>int</info>.',
                $anomaly->message()
            );
        }
